...
const Collections = () => {
  ...
  const filteredProducts = products.filter((p) => {
    ...
    if (selectedTags.length > 0) {
      // One of a kind
      if (selectedTags.includes("One of a Kind") && !p.isLimitedEdition)
        return false;
      // Bespoke
      if (selectedTags.includes("Bespoke") && !p.customCalligraphy)
        return false;
      // Eco-Resin
      if (
        selectedTags.includes("Eco-Resin") &&
        p.materialsUsed &&
        !p.materialsUsed.some((m) => m.toLowerCase().includes("resin"))
      )
        return false;
    }
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      ...
      setCategories([
        { id: "all", name: "All Pieces", slug: "all-pieces" },
        ...data.filter((c) => c.slug !== "all-pieces"),
      ]);
  ...
  return (
    <div className="w-full bg-cream min-h-screen pb-24">
      ...
      <div className="pt-16 pb-12 text-center px-6">
        ...
        >
          Hand-crafted resin art that bridges the gap between raw elements and
          contemporary luxury.
        </motion.p>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8">
          ...
          >
            {/* Search */}
            <div>
              <h4 className="text-xs tracking-widest uppercase text-charcoal/50 mb-3 font-semibold">
                Search
              </h4>
            ...
            {/* Categories */}
            <div>
              <h4 className="text-xs tracking-widest uppercase text-charcoal/50 mb-4 font-semibold">
                Categories
              </h4>
              {loadingCategories ? (
                    ...
                    <li key={cat.id}>
                      ...
                      >
                        ...
                        {activeCategory === cat.slug && (
                          <span className="ml-2 text-rose-gold text-[10px]">
                            ♦
                          </span>
                        )}
                      </button>
            ...
            <div>
              ...
              <div className="flex items-center gap-4">
                <div className="bg-white/50 border border-charcoal/10 px-3 py-2 flex items-center flex-1">
                  <span className="text-charcoal/40 mr-1">EGP</span>
                  <input
                    type="number"
                    placeholder="0"
                    value={minPrice}
          ...
          <div className="md:col-span-9 flex flex-col">
            ...
            >
              <span className="italic font-serif">
                Showing {filteredProducts.length} masterpiece
                ...
                filteredProducts.map((product, idx) => (
                  ...
                  >
                    <Link
                      ...
                      className="block relative aspect-[4/5] overflow-hidden bg-sand/20 mb-4"
                    >
                      <img
                      ...
                      <div className="absolute inset-0 bg-charcoal/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                        <span className="text-cream text-xs uppercase tracking-[0.2em] border-b border-cream pb-1">
                          View Details
                        </span>
                      </div>
                    </Link>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start gap-1 sm:gap-2 mt-2 sm:mt-0">
                      <div className="pr-0 sm:pr-2">
                        <h3 className="text-charcoal font-serif text-sm sm:text-lg mb-0.5 sm:mb-1 leading-tight">
                          {product.name}
                        </h3>
                        <p className="text-charcoal/50 text-[10px] sm:text-xs tracking-widest uppercase">
                          {product.categoryId.replace("-", " ")}
                        </p>
                      </div>
                      <span className="text-rose-gold font-medium text-sm sm:text-base whitespace-nowrap mt-1 sm:mt-0">
                        {product.price} EGP
                      </span>
                    </div>
                  </motion.div>
            ...
            >
              ...
              <button className="w-8 h-8 rounded-full bg-rose-gold/90 text-white flex items-center justify-center font-medium shadow-sm">
                1
              </button>
              <button className="w-8 h-8 rounded-full flex items-center justify-center text-charcoal/60 hover:bg-sand transition-colors">
                2
              </button>
              <button className="w-8 h-8 rounded-full flex items-center justify-center text-charcoal/60 hover:bg-sand transition-colors">
                3
